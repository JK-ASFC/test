  ...
  function checkSopporto() {
    ...
    if (window.innerHeight < 600) {
      heightSpiegazione.innerHTML =
        "ERREUR DE HAUTEUR : Vous avez besoin de <b>600px</b>" +
        "   " +
        "pour afficher cette page";
      AttualeHeightContainer.innerHTML = window.innerHeight + "px";
      heightSpiegazione.style.color = "rgb(255, 0, 47)";
    } else {
      heightSpiegazione.innerHTML = "La bonne hauteur d'écran !";
...
document.addEventListener('DOMContentLoaded', function() {
  ...
  if (userName) {
      document.querySelector('.nome-utente-start-section span').textContent = userName;
  }

  if (profilePic) {
      const userIcon = document.querySelector('.nome-utente-start-section svg');
      userIcon.outerHTML = `<img src="${profilePic}" alt="Profile Picture" style="width: 24px; height: 24px; border-radius: 50%;">`;
  }
});